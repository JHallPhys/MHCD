...
%==========================================================================
Norm_hm=abs(z).^2-1i*gamma*abs(z).^2-beta*abs(z).^4-delta/sqrt(2)*(conj(z)+z);
% zzzz
% Norm_hm=real(Norm_hm)./max(max(real(Norm_hm)));
% Norm_hm=exp(Norm_hm)
% Norm_hm=Norm_hm./max(max(real(Norm_hm)));
% % 
figure
contour(q_i,p_i,real(log(Norm_hm))+imag(log(Norm_hm)),40)
